...
export default class StopView extends React.Component {
    ...
    refreshName() {
        ...
        axios
            .get(
                `${ENDPOINTS['BASE_URL']}${ENDPOINTS['STOP']}${
                    this.props.stopId
                }`
            )
            .then(response => {
                ...
                if (response.data.code != 200 || !response.data.data) {
                    ...
                    this.setState({
                        stopName: `${response.data.data.entry.name} (${
                            response.data.data.entry.direction
                        })`
                    });
                }
            })
    ...
    refreshArrivals() {
        ...
        axios
            .get(
                `${ENDPOINTS['BASE_URL']}${ENDPOINTS['ARRIVALS_DEPARTURES']}${
                    this.props.stopId
                }`
            )
            .then(response => {
                ...
                if (response.data.code != 200 || !response.data.data) {
                    ...
                    const arrDep =
                        response.data.data.entry.arrivalsAndDepartures;
                    arrDep.sort(
                        (a, b) =>
                            (a.predictedDepartureTime ||
                                a.scheduledDepartureTime) -
                            (b.predictedDepartureTime ||
                    ...
                    for (let entry of arrDep) {
                        ...
                        const arrEntity = new Date(
                            entry.predictedDepartureTime ||
                                entry.scheduledDepartureTime
                        );
                        const arrTime = arrEntity.toLocaleTimeString();
                        const timeDiff = Math.floor((arrEntity - now) / 60000);
                        if (timeDiff >= 0) {
                            stopEntries.push(
                                <div key={`${dest}-${i}`}>
            ...
            <div className="stop-wrapper">
                <div className={headingClass}>
                    ...
                    <span
                        ...
                        onClick={() =>
                            this.props.stopDeleter(this.props.stopIndex)
                        }
                    />
                </div>
